...
const BASE_DATOS_INGREDIENTES: IngredienteDB[] = [
    ...
    { palabras: ['calamar', 'sardina', 'atun', 'salmon'], kcal100g: 200, pesoPorUnidad: 150 },
    { palabras: ['merluza', 'bacalao', 'pulpo', 'gamba', 'langostino', 'sepia'], kcal100g: 90, pesoPorUnidad: 150 },
    ...
    { palabras: ['azucar', 'miel', 'chocolate', 'galleta', 'cacao'], kcal100g: 450, pesoPorUnidad: 15, advertencia: 'Alto en azúcares' },
...
// 5. MOTOR PRINCIPAL
export const obtenerNutricionDesdeAPI = async (
    ...
    try {
        ...
        const textoCompleto = normalizar(
            ingredientesStr + " " + (descripcion || "") + " " + tipoReceta
        );

        const esFritura = detectarFritura(textoCompleto);
        ...
        const regexParser =
            /^([\d.,\/]+)?\s*(kg|kilos?|g|gr|gramos|ml|l|litros?|cucharadas?|cda|cucharaditas?|cdta|tazas?|dientes?|pz|piezas?|unidades?)?\s*((?:de\s+)?.*)$/i;

        for (const item of listaIngredientes) {

            if (!item || item.toLowerCase().includes("para")) continue;
            ...
            const alimentoDB = BASE_DATOS_INGREDIENTES.find(dbItem =>
                dbItem.palabras.some(p => {
                    ...
                    return nombreLimpio.includes(base) || nombreLimpio.includes(base + "s");
                })
            );
        ...
        return {
            ...
            meta: {
                ...
                precision:
                    desglose.length + desconocidos.length > 0
